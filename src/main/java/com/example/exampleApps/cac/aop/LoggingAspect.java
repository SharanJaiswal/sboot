...
1. "execution": matches a particular method in a particular class. Access-modifier is optional to mention, and works on all 4 access-modifiers.
    ...
    Use of wildcards in pointcut:
      a. "*" : matches any single element(retType, packageWithClassName, methodName, arguments) with count 1. Example:
        A. "*" at return type to match any return type;
        B. * com.class.path.Class.*(String) :matches with all methods in class Class having 1 param of type String returning any ret type;
        C. String com.class.path.Class.methodName(*) : match given method of given return type having single param of any type.
        D. String com.class.path.Class.methodName(*, *) : match given method of given return type having two param of any type.
...
3. "@within" works like the "within", but it only works with expressions signifying Annotations, considers all the methods of that class having this annotation, not packages or anything.
...
5. to match any method with particular set of argument or param [ @Before("args(String, int)") ] OR [ @Before("args(com.path.to.userDefined.ClassType, ..., int,String,boolean,...)")]

6. To match any method with params and that param ClassType is annotated with particular annotation. @Before("@args(org.springframework.stereotype.Service)") , eg: met1(EmpRepo emprep) {} --- @Service class EmpRepo {}

7. To match any method on a particular instance of a class/interface, ie, whenever any only instance of this class/interface is used to call the method into it, interceptor will get called. @Before("target(com.path.to.Class)")
 ...
 *  1. look for @Aspect annotated classes while loading beans in IoC.
 *  2. parse pointcut expression (done by PointcutParser.java class's parsePointcutExpression method), and store it in a Data structure or cache for efficient matching.
 *  3. look for @Component, @service, @Controller, etc., annotated class.
 *  4. For each class, it checks if its eligible for interception based on pointcut expression. (done by AbstractAutoProxyCreator.java class's wrapIfNecessary method)
 *  5. If yes, it created proxy (proxy child class with overridden intercepted method), using either CGLIB(Code Generation Library Proxy) or Dynamic Proxy. This proxy class has code which execute advice before the method, then method execution happens and after that advice if any.
 *
 *
 *  JDK Dynamic proxy is used when the (eligible matched method) class is implementing an interface. This creates a "new" class implementing same interface but with new method body including advised tasks.
